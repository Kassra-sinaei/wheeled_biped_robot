...
double* Jump::generateTraj(double l_0, double l_d_0, double l_dd_0, 
                double l_to, double t_0, double t_to, double t_recover, double zd){
    ...
    for (double time = t_0; time < t_to; time += this->dt){
        res[i] = coefs[0] + coefs[1] * time + coefs[2] * pow(time, 2) + coefs[3] * pow(time, 3) + 
                + coefs[4] * pow(time, 4) + coefs[5] * pow(time, 5);
        i++;
    }
